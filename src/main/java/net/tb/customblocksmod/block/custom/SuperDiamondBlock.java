...
public class SuperDiamondBlock extends Block {
    public SuperDiamondBlock(Properties pProperties) {
        super(pProperties);
    }
        ...
        if (!pLevel.isClientSide()) {
            ...
            timer.schedule(new TimerTask() {
                ...
                public void run() {
                    ...
                    diamondMansion(pLevel, pPos);
                    pPlacer.sendMessage(new TextComponent("Diamond Mansion"), pPlacer.getUUID());
                }
            }, 1000);
        }
    }

    private void diamondMansion(Level pLevel, BlockPos pPos) {
        ServerLevel serverLevel = pLevel.getServer().getLevel(pLevel.dimension());
        ResourceLocation resourceLocation = new ResourceLocation(CustomBlocksMod.MOD_ID, "diamondmansion/start_pool");
